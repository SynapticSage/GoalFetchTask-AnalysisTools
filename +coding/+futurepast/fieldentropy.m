function out = fieldentropy(field, behfield, varargin)
% function entropy = fieldentropy(field, varargin)
% Calculates the entropy of a field.


ip = inputParser;
ip.addParameter('shiftdim', []);
ip.addParameter('celldim', []);
ip.addParameter('outerdim', []);
ip.addParameter('nBins', []);
ip.addParameter('valuesPerBin', 10);
ip.parse(varargin{:})
Opt = ip.Results;
...
if isnumeric(field)
    ...
    if isempty(Opt.outerdim)
        if Opt.nBins == 0
            ...
            occupancy    = histcounts(behfield(:), Opt.nBins);
            occdistr = occupancy./sum(occupancy, 'all');
            out.occdistr    = occdistr;

            r = histcounts(field(:), Opt.nBins);
        ...
        tmp   = cellfun(@(x) coding.futurepast.dotson.fieldentropy(x), field, "UniformOutput", true);
...
elseif isstruct(field)
    ...
    out = coding.futurepast.dotson.fieldentropy(field.FR_occNorm);
